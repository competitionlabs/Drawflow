...
const closest = function (element, selector) {
  ...
  function closest (element, selector) {
    ...
    if (element !== null && typeof element === 'object') {
      if (selector.match(parameters.classSelector) && hasClass(element, selector)) {
        return element;
      } else if (selector.match(parameters.idSelector) && element.id === selector.replace('#', '')) {
        return element;
      } else if (selector.match(parameters.tagSelector) && element.nodeName === selector.toUpperCase()) {
        return element;
      } else {
